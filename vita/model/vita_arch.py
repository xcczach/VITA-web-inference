...
class VITAMetaModel:
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            self.vision_tower = build_vision_tower(
                config,
                delay_load=False,  # not getattr(config, "continuous_training", False)
            )
    ...
    def initialize_audio_modules(self, model_args):
        ...
        if pretrain_audio_mlp_adapter is not None:
            audio_projector_weights = torch.load(
                pretrain_audio_mlp_adapter, map_location="cpu"
            )
            ...
            self.audio_encoder.adpter.load_state_dict(
                get_w(audio_projector_weights, "audio_encoder.adpter")
            )
...
class VITAMetaForCausalLM(ABC):
    ...
    def pool_feats(self, x, out_size):
        ...
        num_tokens = (
            x.shape[2] * x.shape[3]
        )  # Recalculate the number of tokens after pooling
    ...
    def slow_fast_pooling0(self, temp_img_feats):
        ...
        if num_frame <= 30:
            slow_token_num = max(
                [e for e in [256, 225, 196, 169] if e <= 5200 / num_frame]
            )
        ...
        if num_frame <= 30:
            ...
            num_slow = int(
                (5200 - fast_token_num * num_frame) / (slow_token_num - fast_token_num)
            )
    ...
    def slow_fast_pooling1(self, temp_img_feats):
        ...
        if num_frame <= 28:
            slow_token_num = max(
                [e for e in [256, 225, 196, 169, 144] if e <= 4096 / num_frame]
            )
            fast_token_num = slow_token_num
        elif num_frame <= 40:
            slow_token_num = 144
        ...
        if num_frame <= 28:
            ...
            num_slow = int(
                (4096 - fast_token_num * num_frame) / (slow_token_num - fast_token_num)
            )
        num_fast = num_frame - num_slow
        slow_index = list(np.linspace(0, num_frame, num=num_slow, dtype=int))
    ...
    def slow_fast(self, image_features, sf_masks):
        ...
        for i, img_feat in enumerate(image_features):
            if i == 0 or sf_masks[i] != sf_masks[i - 1]:
                if (
                    temp_img_feats
                ):  # 如果 temp_img_feats 不为空，则添加到 new_image_features
                    if sf_masks[i - 1] > 0:
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        cur_image_idx = 0
        cur_audio_idx = 0
        assert sum([(cur == IMAGE_TOKEN_INDEX).sum() for cur in input_ids]) + sum(
            [(IMAGE_TOKEN_INDEX not in cur) for cur in input_ids]
        ) == len(image_features), input_ids
        assert (
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + num_audio_frames + 1):
                ...
                if i < num_images + num_audio_frames:
                    if (
                        ...
                        if getattr(
                            self.config, "audio_prompt_num", None
                        ):  # self.config.audio_prompt_num:
                            cur_lengths_for_llm = (
                                cur_lengths_for_llm + self.config.audio_prompt_num
                            )
            ...
            # put all tensors in cur_new_input_embeds and cur_new_labels to the same device
            cur_new_input_embeds = [
                x.to(cur_new_input_embeds[0].device) for x in cur_new_input_embeds
            ]
            cur_new_labels = [x.to(cur_new_labels[0].device) for x in cur_new_labels]
        ...
        if state_labels is not None:
            assert sum([(cur == AUDIO_TOKEN_INDEX).sum() for cur in input_ids]) == sum(
                [(cur == -101).sum() for cur in new_labels]
            ) + sum([(cur == -102).sum() for cur in new_labels]), (
                input_ids,
                sum([(cur == AUDIO_TOKEN_INDEX).sum() for cur in input_ids]),
                sum([(cur == -101).sum() for cur in new_labels]),
                sum([(cur == -102).sum() for cur in new_labels]),
                new_labels.shape,
            )
